...
void Bridge::letsDebug()
{
	...
	depthVector sim0, sim1, sim2;
        ...
        debug << endl;
        debug << "Similarity for wikicategory_American_cyclists : " << sim0.score(Param::M) << endl;
        debug << '\t';
        for (int i = (int) sim0.w.size() - 1; i >= 0; i --)
                debug << sim0.w[i] << " ";
        debug << endl << endl;
        debug << "Similarity for wikicategory_Paralympic_cyclists_of_Australia : " << sim1.score(Param::M) << endl;
        debug << '\t';
	...
	debug << "Total number of entities wikicategory_Belfast_Giants_players has : "
		...
		<< endl << endl;
	debug << "Similarity for wikicategory_MHK_Kežmarok_players : " << sim0.score(Param::M) << endl;
	debug << '\t';
	for (int i = (int) sim0.w.size() - 1; i >= 0; i --)
		debug << sim0.w[i] << " ";
	debug << endl << endl;
	debug << "Similarity for wikicategory_Belfast_Giants_players : " << sim1.score(Param::M) << endl;
	debug << '\t';
	for (int i = (int) sim1.w.size() - 1; i >= 0; i --)
		debug << sim1.w[i] << " ";
	debug << endl << endl;
	...
	debug << endl;
	debug << "Similarity for wikicategory_Bridges_on_the_National_Register_of_Historic_Places_in_Georgia_(U.S._state) : " << sim0.score(Param::M) << endl;
	debug << '\t';
	for (int i = (int) sim0.w.size() - 1; i >= 0; i --)
		debug << sim0.w[i] << " ";
	debug << endl << endl;
	debug << "Similarity for wikicategory_Road_bridges_in_Oregon : " << sim1.score(Param::M) << endl;
	debug << '\t';
	for (int i = (int) sim1.w.size() - 1; i >= 0; i --)
		debug << sim1.w[i] << " ";
	debug << endl << endl;
	debug << "Similarity for wikicategory_Bridges_in_Portland,_Oregon : " << sim2.score(Param::M) << endl;
	debug << '\t';
